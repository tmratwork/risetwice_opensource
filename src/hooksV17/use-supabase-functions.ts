// src/hooksV17/use-supabase-functions.ts
// V17 Supabase Function Loading Service (Eleven Labs)
...
// Import V16 mental health functions for V17 compatibility
import { useMentalHealthFunctionsV16 } from '@/hooksV16';
import type { ElevenLabsStoreState } from '@/stores/elevenlabs-store';
...
async function executeFunctionImplementation(functionName: string, args: unknown, mentalHealthFunctions: ReturnType<typeof useMentalHealthFunctionsV16>): Promise<unknown> {
  ...
  switch (functionName) {
    case 'trigger_specialist_handoff':
      return await executeTriggerSpecialistHandoffV17(args);
 ...
 */
// V16 function kept for reference - V17 uses executeTriggerSpecialistHandoffV17
...
/**
 * V17 Execute the trigger_specialist_handoff function for Eleven Labs
 * This is the key V17 function that enables triage AI handoffs using Eleven Labs
 */
async function executeTriggerSpecialistHandoffV17(args: unknown): Promise<unknown> {
  const triggerTime = performance.now();
  const sessionId = Date.now().toString(36) + Math.random().toString(36).substr(2);
  ...
  // Detect who is initiating the handoff (V17 uses Eleven Labs store)
  let currentSpecialist = 'unknown';
  if (typeof window !== 'undefined' && (window as unknown as { useElevenLabsStore: { getState: () => ElevenLabsStoreState } }).useElevenLabsStore) {
    const elevenLabsState = (window as unknown as { useElevenLabsStore: { getState: () => ElevenLabsStoreState } }).useElevenLabsStore.getState();
    
    // Get current specialist from Eleven Labs store or conversation state
    const triageSession = elevenLabsState.triageSession;
    if (triageSession?.currentSpecialist) {
      currentSpecialist = triageSession.currentSpecialist;
    }
  }

  logV17('===== V17 SPECIALIST HANDOFF INITIATED =====', {
    sessionId,
    currentSpecialist,
    triggerTime: triggerTime.toFixed(3) + 'ms',
    provider: 'eleven-labs',
    args
  });

  try {
    // Parse the handoff parameters
    const params = args as {
      specialist_type: string;
    ...
    logV17('📋 Handoff parameters parsed', {
      specialist_type: params.specialist_type,
      handoff_reason: params.handoff_reason,
      context_length: params.context_summary?.length || 0,
      urgency_level: params.urgency_level || 'normal'
    });

    // Validate parameters
    if (!params.specialist_type || !params.handoff_reason || !params.context_summary) {
      const errorMsg = 'Missing required handoff parameters (specialist_type, handoff_reason, or context_summary)';
      logV17('❌ Validation failed', { error: errorMsg, params });
      return {
        success: false,
        error: errorMsg
      };
    ...
    // Get conversation ID from localStorage (V17 pattern)
    let conversationId = '';
    if (typeof window !== 'undefined') {
      conversationId = localStorage.getItem('currentConversationId') || '';
    }

    logV17('🔄 Preparing V17 specialist handoff event', {
      conversationId,
      specialist_type: params.specialist_type,
      from_specialist: currentSpecialist
    });

    // Create the specialist handoff event (compatible with V16 pattern)
    const handoffEvent = new CustomEvent('specialist_handoff', {
      detail: {
        specialist: params.specialist_type,
        context: params.context_summary,
        reason: params.handoff_reason,
        urgency: params.urgency_level || 'normal',
        timestamp: new Date().toISOString(),
        sessionId,
        conversationId,
        fromSpecialist: currentSpecialist,
        provider: 'eleven-labs',
        version: 'V17'
      }
    });
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(handoffEvent);
      logV17('📡 V17 specialist handoff event dispatched', {
    ...
    logV17('✅ V17 handoff function completed successfully', {
      sessionId,
      specialist_type: params.specialist_type,
      context_length: params.context_summary.length
    });

    return {
      success: true,
      message: `Handoff initiated to ${params.specialist_type} specialist`,
      data: {
        specialist_type: params.specialist_type,
        handoff_reason: params.handoff_reason,
        context_summary: params.context_summary,
    ...
    logV17('❌ ERROR in V17 trigger_specialist_handoff', {
      error: (error as Error).message,
      stack: (error as Error).stack
    });
    return {
      success: false,
      error: `V17 handoff failed: ${(error as Error).message}`
    };
  }
}
