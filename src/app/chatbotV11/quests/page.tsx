// src/app/chatbotV11/quests/page.tsx
...
import '../chatbotV11.css'; // Import the shared CSS file
...
interface Quest {
  ...
  ai_prompt?: string; // Added ai_prompt field as optional since it was added manually to the DB
...
export default function QuestsPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (storedBookId) {
        ...
        // If no book is selected, use a default psychology textbook ID
        const defaultBookId = '2b169bda-011b-4834-8454-e30fed95669d';
        console.log('No book found in localStorage, using default:', defaultBookId);
        localStorage.setItem('selectedBookId', defaultBookId);
  ...
  const startQuest = async (questId: string) => {
    ...
    try {
      ...
      console.log('🚀 Passing quest data to chatbot page:', {
      ...
      // Store the full quest object in sessionStorage (better than localStorage for temporary data)
  ...
  // Enhanced debugging based on WebAI recommendations - check panel rendering
  useEffect(() => {
    const enhancedDebug = () => {
      console.log('[UI-INSIGHTS] 🚀 Enhanced debug function started');
      console.log('[UI-INSIGHTS] === ENHANCED PANEL RENDERING CHECK ===');

      // Check if all expected panels exist in DOM
      const privacyPanel = document.querySelector('[data-panel="privacy"]');
      const insightsPanel = document.querySelector('[data-panel="insights"]');
      const summaryPanel = document.querySelector('[data-panel="summary"]');
      const aiRemembersPanel = document.querySelector('[data-panel="ai-remembers"]');
      const profileControls = document.querySelector('#profile-controls');

      // Find Profile Controls by text content if ID doesn't work
      let profileControlsByText = null;
      const allH3s = document.querySelectorAll('h3');
      for (const h3 of allH3s) {
        if (h3.textContent?.includes('Profile Controls')) {
          profileControlsByText = h3;
          break;
        }
      }

      console.log('[UI-INSIGHTS] Panel existence check:', {
        privacy: !!privacyPanel,
        insights: !!insightsPanel,
        summary: !!summaryPanel,
        aiRemembers: !!aiRemembersPanel,
        profileControls: !!profileControls,
        profileControlsByText: !!profileControlsByText
      });

      // If Profile Controls exists, check its position
      const targetElement = profileControls || profileControlsByText;
      if (targetElement) {
        const rect = targetElement.getBoundingClientRect();
        const computed = getComputedStyle(targetElement);
        console.log('[UI-INSIGHTS] Profile Controls found:', {
          rect: {
            top: rect.top,
            left: rect.left,
            bottom: rect.bottom,
          ...
          offsetParent: (targetElement as HTMLElement).offsetParent?.tagName,
          offsetTop: (targetElement as HTMLElement).offsetTop,
          scrollTop: window.scrollY
        });
      } else {
        console.log('[UI-INSIGHTS] ❌ Profile Controls not found in DOM');
      }

      // Check for any elements with suspicious positioning
      const allElements = document.querySelectorAll('*');
      const suspiciousElements = Array.from(allElements).filter(el => {
        const rect = el.getBoundingClientRect();
        return rect.top > 5000 || rect.left > 5000; // Elements positioned way off screen
      });

      if (suspiciousElements.length > 0) {
        console.log('[UI-INSIGHTS] 🚨 Found suspiciously positioned elements:',
          suspiciousElements.slice(0, 5).map(el => ({
            element: el.tagName,
            className: el.className?.substring(0, 50) || 'no-class',
      ...
      const mainContent = document.querySelector('.mb-96');
      if (mainContent) {
        const rect = mainContent.getBoundingClientRect();
        console.log('[UI-INSIGHTS] Main content container:', {
          height: rect.height,
          top: rect.top,
          bottom: rect.bottom,
      ...
      console.log('[UI-INSIGHTS] === PANEL HEIGHT ANALYSIS ===');
      const panels = [
        { selector: '.bg-gray-100.dark\\:bg-gray-800', name: 'Privacy Settings Panel' },
        { selector: '.bg-white.dark\\:bg-gray-800.border', name: 'Insights/Summary/AI Remembers Panels' },
        { selector: '#ai-remembers-section', name: 'AI Remembers Section' }
      ];
      ...
      panels.forEach(panel => {
        ...
        elements.forEach((el, index) => {
          const rect = el.getBoundingClientRect();
          console.log(`[UI-INSIGHTS] ${panel.name} ${index}:`, {
            height: rect.height,
            top: rect.top,
            bottom: rect.bottom,
      ...
      const tabNav = document.querySelector('.fixed.bottom-16');
      if (tabNav) {
        const rect = tabNav.getBoundingClientRect();
        console.log('[UI-INSIGHTS] Tab nav top:', rect.top);
        console.log('[UI-INSIGHTS] Tab nav height:', rect.height);
      }

      console.log('[UI-INSIGHTS] === ENHANCED DEBUG COMPLETE ===');
    };

    if (activeTab === 'insights') {
      console.log('[UI-INSIGHTS] Enhanced debug function scheduled to run in 1000ms');
      setTimeout(() => {
        console.log('[UI-INSIGHTS] Enhanced debug function executing now');
        enhancedDebug();
      }, 1000);
    }
  }, [activeTab]);

  // Complete insights data loading function (from original insights page)
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      {activeQuests.length === 0 ? (
                        <div className="text-center py-4 text-gray-500 dark:text-gray-300">
                          You have no active quests. Start a new quest below!
                        </div>
                      ) : (
                    ...
                    <div>
                      ...
                      {otherQuests.length === 0 ? (
                        <div className="text-center py-4 text-gray-500 dark:text-gray-300">
                          No other quests available for this book
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {insightsLoading ? (
                  ...
                  <>
                    ...
                    <div
                      ...
                      ref={(el) => {
                        if (el) {
                          console.log('[PANEL-DEBUG] Privacy panel rendered, height:', el.offsetHeight);
                        }
                      }}
                    >
                    ...
                    {summarySheetData && (
                      <div id="summary-sheet-section" className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-6 rounded-lg shadow-md mb-6 relative z-10">
                        ...
                        <div className="mt-10 pt-4 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400">
                          {(summarySheetData.content as { footer?: string }).footer || 'This summary was generated to help facilitate a warm hand-off to a support provider.'}
                        </div>
                      </div>
                    )}
                            ...
                            {userProfile && (
                              <div
                                ...
                                ref={(el) => {
                                  if (el) {
                                    const rect = el.getBoundingClientRect();
                                    const aiRemembersSection = document.querySelector('[data-panel=\"ai-remembers\"]');

                                    console.log('[UI-INSIGHTS] PROFILE CONTROLS DISPLACEMENT DEBUG');
                                    console.log('[UI-INSIGHTS] Profile Controls position:', {
                                    ...
                                    if (aiRemembersSection) {
                                      const aiRect = aiRemembersSection.getBoundingClientRect();
                                      console.log('[UI-INSIGHTS] AI Remembers section:', {
                                        top: aiRect.top,
                                        bottom: aiRect.bottom,
                                        height: (aiRemembersSection as HTMLElement).offsetHeight,
                                        contains: aiRemembersSection.contains(el)
                                      });

                                      console.log('[UI-INSIGHTS] Displacement calculation:', {
                                        displacement: rect.top - aiRect.bottom,
                                        shouldBeInside: aiRemembersSection.contains(el)
                                      });
                                    }
                                    ...
                                    while (current && level < 5) {
                                      ...
                                      console.log(`[UI-INSIGHTS] Parent-${level}:`, {
                                        tagName: current.tagName,
                                        className: current.className?.substring(0, 30) || 'no-class',
