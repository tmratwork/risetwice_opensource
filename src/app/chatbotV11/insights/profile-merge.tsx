...
export default function ProfileMergeTab() {
  ...
  useEffect(() => {
    ...
    const fetchPrompts = async () => {
      try {
        ...
        const response = await fetch('/api/v11/prompts?category=profile_merge', {
          headers: {
        ...
        if (response.ok) {
          ...
          const system = data.prompts.filter((p: PromptData) => p.category === 'profile_merge_system');
          const user = data.prompts.filter((p: PromptData) => p.category === 'profile_merge_user');
  ...
  const handleSelectPrompt = (prompt: PromptData) => {
    ...
    setFormData({
      ...
      category: activeTab === 'system' ? 'profile_merge_system' : 'profile_merge_user'
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedResponse = await fetch('/api/v11/prompts?category=profile_merge', {
          headers: {
            'Content-Type': 'application/json',
            ...(idToken ? { 'Authorization': `Bearer ${idToken}` } : {})
          }
        ...
        if (updatedResponse.ok) {
          ...
          const system = updatedData.prompts.filter((p: PromptData) => p.category === 'profile_merge_system');
          const user = updatedData.prompts.filter((p: PromptData) => p.category === 'profile_merge_user');
