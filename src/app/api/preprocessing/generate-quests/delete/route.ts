...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (questIds && questIds.length > 0) {
      ...
      // Delete all user_quests records for these quest IDs
      const { data, error: userQuestsError } = await supabase
        .from('user_quests')
        .delete()
        .in('quest_id', ids)
        .select('count');
      ...
      if (userQuestsError) {
        ...
        userQuestsDeletedCount = data?.length || 0;
        console.log(`${logPrefix} Deleted ${userQuestsDeletedCount} user quest records`);
      }
    } else {
      console.log(`${logPrefix} No quests found for this book`);
    }

    // Step 3: Delete all book_quests records for this book
    const { data: deletedQuests, error: deleteError } = await supabase
      .from('book_quests')
      .delete()
      .eq('book_id', bookId)
      .select('count');
    ...
    const questsDeletedCount = deletedQuests?.length || 0;
