...
export async function GET(request: NextRequest) {
  try {
    ...
    const conversations = messages?.reduce((acc, msg) => {
      ...
      return acc;
    }, {} as Record<string, { intakeId: string; accessCode: string; providerUserId: string; messages: Array<{ id: string; senderType: string; audioUrl: string; durationSeconds: number; readAt: string | null; createdAt: string }> }>);
