// src/app/api/v11/book-content/route.ts
/**
 * V11 API - Book Content Query Endpoint
 * 
...
export async function POST(req: Request) {
  ...
  const logPrefix = `[V11-API-BookContent][${requestId}]`;

  console.log('===============================================')
  console.log(`${logPrefix} === STARTING BOOK CONTENT QUERY ===`);
  console.log('===============================================')
  
  // Debug handler to log all incoming requests
  console.log(`${logPrefix} INCOMING REQUEST HEADERS:`, {
  ...
  try {
    ...
    if (!book) {
      ...
      console.warn(`${logPrefix} Full request body was:`, JSON.stringify(body, null, 2));
      return NextResponse.json(
        { 
          error: 'Book ID is required',
          details: `Book ID field (book) was ${book === null ? 'null' : 'undefined'} in request`
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      console.log(`[pinecone ${logPrefix} Getting index details for: ${process.env.PINECONE_INDEX}`);
      // Add this to inspect index properties
      const indexDetails = await pinecone.describeIndex(process.env.PINECONE_INDEX!);
      console.log(`[pinecone ${logPrefix} Index details:`, indexDetails);

      // Verify Pinecone connection before proceeding
      try {
        console.log(`[pinecone ${logPrefix} Verifying Pinecone connection and listing indexes`);
        ...
        if (!indexExists) {
          console.error(`[pinecone ${logPrefix} The specified Pinecone index '${process.env.PINECONE_INDEX}' does not exist!`);
        } else {
          console.log(`[pinecone ${logPrefix} Confirmed index '${process.env.PINECONE_INDEX}' exists`);
        }
      } catch (connectionError) {
        console.error(`[pinecone ${logPrefix} Failed to connect to Pinecone:`, connectionError);
      ...
      console.log(`[pinecone ${logPrefix} Getting index statistics for namespace analysis`);
      // Add this to check all namespaces and vector counts
      const stats = await index.describeIndexStats();
      console.log(`[pinecone ${logPrefix} Index stats:`, stats);
      ...
      console.log(`[pinecone ${logPrefix} Executing vector similarity query with topK=20, includeMetadata=true`);
      ...
      if (!queryResponse.matches || queryResponse.matches.length === 0) {
        console.error(`[pinecone ${logPrefix} *** tmr240 No matches found in Pinecone, this is a critical error`);
      ...
      if (queryResponse.matches && queryResponse.matches.length > 0) {
        console.log(`[pinecone ${logPrefix} Processing ${queryResponse.matches.length} matches for relevance filtering`);
        ...
        if (filteredMatches.length === 0) {
          ...
          console.log(`[pinecone ${logPrefix} Using ${filteredMatches.length} filtered matches for content generation`);
        }
      }

      // Step 5: Format the response with content from matches
      console.log(`${logPrefix} Starting to format content from ${queryResponse.matches.length} matches`);
      
      // Debug log to see what's in the matches
      console.log(`${logPrefix} Match metadata sample:`, 
        queryResponse.matches.slice(0, 3).map(match => ({
          score: match.score,
          id: match.id,
          hasMetadata: !!match.metadata,
          hasText: match.metadata?.text ? true : false,
          textLength: match.metadata?.text && typeof match.metadata.text === 'string' ? match.metadata.text.length : 0,
          metadataKeys: match.metadata ? Object.keys(match.metadata) : []
        }))
      );
      
      // Count matches with text
      const matchesWithText = queryResponse.matches.filter(match => match.metadata?.text).length;
      console.log(`${logPrefix} Matches with text property: ${matchesWithText}/${queryResponse.matches.length}`);
      ...
      const formattedContent = queryResponse.matches
        ...
        .map((match, index) => {
          ...
          // Add detailed logging for this specific match
          console.log(`${logPrefix} Processing match ${index+1}/${matchesWithText}:`, {
            id: match.id,
            score: match.score, 
      ...
      console.log(`${logPrefix} Final formatted content length: ${formattedContent.length} characters`);
      if (formattedContent.length === 0) {
        console.error(`${logPrefix} WARNING: Formatted content is empty despite having ${queryResponse.matches.length} matches and ${matchesWithText} with text property`);
      }
