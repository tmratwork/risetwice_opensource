...
export default function ResourcesV17Page() {
  ...
  const logMeasurements = (phase: string) => {
    ...
    // console.log(`[layout-measurements] ResourcesV17Page: phase=${phase}, timestamp=${timestamp}`);
    void phase;
    if (gridRoot) {
    ...
    if (footer) {
      ...
      // console.log(`[layout-footer] ResourcesV17Page: top=${rect.top}, bottom=${rect.bottom}, viewportHeight=${window.innerHeight}, visible=${rect.bottom <= window.innerHeight}`);
    }
  ...
  useEffect(() => {
    // console.log(`[layout-lifecycle] ResourcesV17Page: state=mounting, timestamp=${Date.now()}`);
    ...
    // Scroll logging  
    const handleScroll = () => {
      // console.log(`[layout-scroll] ResourcesV17Page: documentScrollTop=${document.documentElement.scrollTop}, timestamp=${Date.now()}`);
    };

    const handleMainContentScroll = () => {
      const mainContent = document.querySelector('.main-content-row');
      if (mainContent) {
        // console.log(`[layout-scroll] ResourcesV17Page: main-content-row scrollTop=${(mainContent as HTMLElement).scrollTop}, timestamp=${Date.now()}`);
      }
    };
    ...
    // console.log(`[layout-lifecycle] ResourcesV17Page: state=static-content-loaded, selectedCategory=${selectedCategory}, timestamp=${Date.now()}`);
  ...
  useEffect(() => {
    // console.log(`[layout-lifecycle] ResourcesV17Page: state=category-changed, selectedCategory=${selectedCategory}, timestamp=${Date.now()}`);
  ...
  const handleResourceSelection = (resourceCard: ResourceFunction, parameters: Record<string, unknown>) => {
    ...
    // Navigate to V17 chat page (updated from V16)
    logResourceGreeting('🎯 SELECTION: Navigating to /chatbotV17');
