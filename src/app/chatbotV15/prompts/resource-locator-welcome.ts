// src/app/chatbotV15/prompts/resource-locator-welcome.ts
...
/**
 * Function to fetch resource greeting from database (reuses V11 API)
 */
async function fetchResourceGreeting(userId?: string): Promise<string> {
  try {
    ...
    console.error('[V15-ResourceLocator] Error fetching greeting from database:', error);
  }
...
export async function getResourceWelcomeContent(
  ...
  console.log('[V15-ResourceLocator] getResourceWelcomeContent called with:', resourceData);
...
- "Let me help you find ${resourceData.title}. What's your biggest concern about getting this help right now?"`;
  ...
  console.log('[V15-ResourceLocator] Generated welcome content length:', welcomeContent.length);
